...
export default function ChatBox({ open, setOpen }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint = (process.env.NEXT_PUBLIC_CHAT_API || defaultEndpoint).trim();
    const isDefaultEndpoint = endpoint === defaultEndpoint;
    ...
    try {
      ...
      if (!res.ok) {
        const errMsg = data?.reply || rawText || `Request failed with status ${res.status}`;
        if (res.status === 404 && isDefaultEndpoint) {
          markError("Chat service not running. Deploy /api/chat or set NEXT_PUBLIC_CHAT_API.");
          return;
        }
    ...
    >
      <div className="flex flex-col h-full bg-slate-100 border border-blue-200 rounded-xl overflow-hidden shadow-lg">
        <header className="flex items-center justify-between px-3 py-2 bg-blue-600 text-white">
          ...
          <button onClick={() => setOpen(false)} className="p-1 hover:bg-white/10 rounded-md">
            <X size={16} />
          </button>
        </header>
        ...
        <form onSubmit={handleSubmit} className="px-3 pb-3 pt-2 bg-slate-100 border-t border-blue-100">
